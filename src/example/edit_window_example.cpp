...
int main(int, const char **)// NOLINT(readability-named-parameter)
{
  constexpr ImGuiWrapConfig config{};// Using defaults.
  return imgui_main(config, my_render_fn);
}

// This variable decides whether to show the "Window Flag" edit window.
namespace {
bool editWindow1{ true };
bool editWindow2{ false };
bool editTable1{ true };
constexpr size_t BoardDim = 3;

// Board itself.
std::array<std::string, BoardDim * BoardDim> board{};
}// namespace
    ...
    ImGuiWrapperReturnType my_render_fn()
{
  ...
  dear::MainMenuBar() && []() {// Begin a MenuBar and execute the code in this scope if the bar is rendered.
    dear::Menu("File") && []() {// Begin a Menu and execute the code in this scope if the menu is rendered.
      ...
      dear::ItemTooltip() && []() { dear::Text("Quit the application"); };
    };
