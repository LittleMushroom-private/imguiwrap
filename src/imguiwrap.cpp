...
int imgui_main(const ImGuiWrapConfig &config, const ImGuiWrapperFn &mainFn, const ImGuiWrapperInitFn &initFn) noexcept
{
  // Setup window
  ...
  GLFWwindow *window = glfwCreateWindow(config.width_, config.height_, config.windowTitle_, nullptr, nullptr);
  ...
  if (config.keyboardNav_) {
    ImGui::GetIO().ConfigFlags |=
      static_cast<decltype(ImGui::GetIO().ConfigFlags)>(ImGuiConfigFlags_NavEnableKeyboard);// Enable Keyboard Controls
  }
  ...
  if (config.enableViewport_) {
    ImGui::GetIO().ConfigFlags |= ImGuiConfigFlags_ViewportsEnable;// Enable Docking
  }
...
void flagsWindow(const char *title, bool *showing, const std::function<void(void)> &impl) noexcept
{
  ...
  constexpr ImGuiWindowFlags editWindowFlags = ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoFocusOnAppearing;
...
void EditTableFlags(const char *title, bool *showing, ImGuiTableFlags *flags) noexcept
{
  ...
  flagsWindow(title, showing, [=]() noexcept {
    ...
    // }
    const auto* const iterator = std::find_if(sizes.begin() + 1, sizes.end(), [sizeFlag](const auto &size) { return size.flag == sizeFlag; });
    if (iterator != sizes.end()) { sizeSelected = std::distance(sizes.begin(), iterator); }
